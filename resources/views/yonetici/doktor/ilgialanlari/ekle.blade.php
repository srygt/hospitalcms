...
@if(isset($ilgialanlari->id))
    @section('title', 'İlgi Alanı Güncelle')
@else
    @section('title', 'İlgi Alanı Ekle')
@endif
    ...
    <div class="row page-titles">
        ...
        <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="javascript:void(0)">İlgi Alanı Ekle</a></li>
            </ol>
        </div>
    </div>
    <!-- row -->
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title"><i class="fa fa-arrow-right"></i> İlgi Alanı Kayıt Formu</h4>
                    <a href="{{route('yonetici.doktor.ilgialanlari.liste')}}" class="btn btn-info pull-right p-10 m-1"><i class="fa fa-list"></i> İlgi Alanı Listesi</a>
                </div>
            ...
            @endif                     
                <div class="card-body">
                    <div class="form-validation">
                        ...
                        action="{{ isset($ilgialanlari->id)
                                    ? route('yonetici.doktor.ilgialanlari.guncelle.get', ['id' => $ilgialanlari->id])
                                    : route('yonetici.doktor.ilgialanlari.store.post') }}"
                        method="post"
                    >
                        @csrf
                        <div class="row">
                                ...
                                <div class="form-group row">
                                    ...
                                    <select class="form-control" required name="doktorid" id="hastaneid">
                                        ...
                                        @foreach($doktorlar as $d)
                                            <option
                                                value="{{ $d->id }}"
                                                @if(old('doktorid', $ilgialanlari->doktorid) === $d->id)
                                                    selected
                                                @endif
                                ...
                                <div class="form-group row">
                                    ...
                                    <select class="form-control" required name="dilid" id="single-select">
                                        ...
                                        @foreach($dil as $d)
                                            <option
                                                value="{{ $d->sembol }}"
                                                @if(old('dilid', $ilgialanlari->dilid) === $d->sembol)
                                                    selected
                                                @endif
                                            >{{ $d->diladi }}</option>
                                        @endforeach
                            ...
                            <div class="col-md-12">
                                <div class="form-group row">
                                    ...
                                    <input class="form-control" id="alanadi" name="alanadi" value="{{ old('alanadi', $ilgialanlari->alanadi ?? '') }}" type="text" placeholder="Alan Adını Yazınız">
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group row">
                                    ...
                                    <select class="form-control" required name="durumu" id="single-select">
                                            <option
                                                value="1"
                                                @if(old('durumu', $ilgialanlari->durumu) === 1)
                                                    selected
                                                @endif
                                            >Aktif</option>
                                            <option
                                                value="0"
                                                @if(old('durumu', $ilgialanlari->durumu) === 0)
                                                    selected
                                                @endif
                                            >Pasif</option>                                            
                                    </select>      
