        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{route('yonetici.tanim.icerikkategori.liste')}}" class="btn btn-info pull-right p-10 m-1"><i class="fa fa-list"></i> Kategori Listesi</a>
                </div>
            ...
            @endif                     
                <div class="card-body">
                    <div class="form-validation">
                        ...
                        action="{{ isset($icerikkategori->id)
                                    ? route('yonetici.tanim.icerikkategori.guncelle.get', ['id' => $icerikkategori->id])
                                    : route('yonetici.tanim.icerikkategori.store.post') }}"
                        method="post"
                    >
                        @csrf
                        <div class="row">
                            <div class="col-md-6 pr-5">
                                <div class="form-group row">
                                    ...
                                    <select class="form-control" required name="hastaneid" id="hastaneidm">
                                        @foreach($hastane as $d)
                                            <option
                                                value="{{ $d->id }}"
                                                @if(old('hastaneid', $icerikkategori->hastaneid) === $d->id)
                                                    selected
                                                @endif
                                            >{{ $d->adi }}</option>
                                        @endforeach
                                ...
                                <div class="form-group row">
                                    ...
                                    <select class="form-control" required name="dilid" id="single-select">
                                        ...
                                        @foreach($dil as $d)
                                            <option
                                                value="{{ $d->sembol }}"
                                                @if(old('dilid', $icerikkategori->dilid) === $d->sembol)
                                                    selected
                                                @endif
                                            >{{ $d->diladi }}</option>
                                        @endforeach
                            ...
                            <div class="col-md-6 pr-5">
                                <div class="form-group row">
                                    ...
                                    <input class="form-control" id="kategoriadi" name="kategoriadi" onload="convertToSlug(this.value)" onkeyup="convertToSlug(this.value)" value="{{ old('kategoriadi', $icerikkategori->kategoriadi ?? '') }}" type="text" placeholder="Kategori Adını Yazınız">
                                    <input class="form-control" id="hlink" name="hlink" value="{{ old('hlink', $icerikkategori->hlink ?? '') }}" type="hidden" placeholder="Kategori Adını Yazınız">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group row">
                                    ...
                                    <select class="form-control" required name="durumu" id="single-select">
                                            <option
                                                value="1"
                                                @if(old('durumu', $icerikkategori->durumu) === 1)
                                                    selected
                                                @endif
                                            >Aktif</option>
                                            <option
                                                value="0"
                                                @if(old('durumu', $icerikkategori->durumu) === 0)
                                                    selected
                                                @endif
                                            >Pasif</option>                                            
                                    </select>      
